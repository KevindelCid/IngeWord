...
public class antesIntro extends AppCompatActivity {
   ...
   private TextView edtex;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_antes_intro);
        ...
        edtex = findViewById(R.id.textoref);

        if (ArchivoExiste(archivos, proyectname() + "_ref_Intro.txt")) {
            try {
                InputStreamReader archivo = new InputStreamReader(openFileInput(proyectname() + "_ref_Intro.txt"));
    ...
    private boolean ArchivoExiste(String archivos [], String NombreArchivo){
        ...
        for(int i =0; i<archivos.length; i++)
            if((archivos[i]).equals(proyectname()+"_ref_Intro.txt"))
                return true;
        return false;
